...
public class TaiKhoanPanel extends javax.swing.JPanel {
    ...
    private QuanLyTaiKhoanBUS qltkBUS = new QuanLyTaiKhoanBUS();
    /**
     * Creates new form SanPhamPanel
     */
    public TaiKhoanPanel() {
        initComponents();
        mdtb1 = (DefaultTableModel) tbTaiKhoan.getModel();
        setDataToTable(qltkBUS.getDstk());
    ...
    private void initComponents() {
        ...
        cbTypeSearch.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Tất cả", "Tên tài khoản", "Mật khẩu", "Mã nhân viên", "Mã quyền" }));
        ...
        tbTaiKhoan.setFont(new java.awt.Font("Segoe UI", 0, 16)); // NOI18N
        tbTaiKhoan.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "STT", "Tên tài khoản", "Mật khẩu", "Mã nhân viên", "Loại tài khoản"
            }
        ));
        tbTaiKhoan.setFillsViewportHeight(true);
        tbTaiKhoan.setRowHeight(40);
        tbTaiKhoan.getTableHeader().setFont(new Font("Segoe UI", Font.BOLD, 16));
        tbTaiKhoan.setShowGrid(true);
        jScrollPane1.setViewportView(tbTaiKhoan);
        if (tbTaiKhoan.getColumnModel().getColumnCount() > 0) {
            tbTaiKhoan.getColumnModel().getColumn(0).setMaxWidth(80);
        }
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        themtk.addWindowListener(new java.awt.event.WindowAdapter() {
            ...
            public void windowClosed(java.awt.event.WindowEvent windowEvent) {
                qltkBUS.readDB();
                setDataToTable(qltkBUS.getDstk()); // refresh
            }
        });
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        try {
            String tentk = (String) tbTaiKhoan.getValueAt(tbTaiKhoan.getSelectedRow(), 1);
            if (tentk != null) {
                ThemSuaTaiKhoanForm suatk = new ThemSuaTaiKhoanForm("Sửa", tentk);
    ...
    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        try {
            String tentk = (String) tbTaiKhoan.getValueAt(tbTaiKhoan.getSelectedRow(), 1);
            if (tentk != null) {
                if (JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa tài khoản " + tentk + " ?", "Chú ý", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                    new QuanLyTaiKhoanBUS().delete(tentk);
                    qltkBUS.readDB();
                    setDataToTable(qltkBUS.getDstk()); // refresh
                }
            } 
    ...
    private void setDataToTable(ArrayList<TaiKhoan> data) {
        mdtb1.setRowCount(0); // xóa table hiện có
        int stt = 1; // lưu số thứ tự dòng hiện tại
        for (TaiKhoan tk : data) {
            mdtb1.addRow(new String[]{String.valueOf(stt), tk.getUsername(), tk.getPassword(), tk.getMaNV(), tk.getLoaiQuyen()});
            stt++;
        }
    }
    
    private void tfSearchOnChange() {
        setDataToTable(qltkBUS.search(tfTim.getText(), cbTypeSearch.getSelectedItem().toString()));
    }
